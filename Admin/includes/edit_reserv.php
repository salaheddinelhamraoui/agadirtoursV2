...
$edit_reserv = mysqli_query($con,"select * from reservation where IdReservation='$_GET[IdReservation]'");

$fetch_edit = mysqli_fetch_array($edit_reserv);
...
?>
   ...
   ?></td>
	...
	<br></br>
	   <tr>
			   <td></td>
		   <td colspan="7"><button class="button button1" type="submit" name="edit_reserv" style="font-family:'Great Vibes', cursive;";>Sauvegarder</button></td>
		</tr>
...
if(isset($_POST['edit_reserv'])){
   $statut = trim(mysqli_real_escape_string($con,$_POST['statut']));



   $update_reserv = mysqli_query($con,"update reservation set statut='$statut' where IdReservation='$_GET[IdReservation]' ");
   
 if($update_reserv){
   
   echo "<script>alert('La réservation a été mis à jour avec succès!')</script>";
